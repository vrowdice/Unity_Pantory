...
특히, 플레이어의 몰입과 자급자족 수직계열화 성취감 극대화를 위해 **모든 자원은 완전히 0개(빈손) 상태로 시작**합니다.
...
|----|------|-----------|---------|--------|
| aluminum_ore | raw | 78 | 0 | - |
| coal | raw | 26 | 0 | - |
| copper_ore | raw | 52 | 0 | - |
| gold_ore | raw | 208 | 0 | - |
| iron_ore | raw | 36 | 0 | - |
| oil | raw | 42 | 0 | - |
| wood_log | raw | 29 | 0 | - *(primaryOutputPerBatch: 2)* |
| aluminum_ingot | metal | 247 | 0 | aluminum_ore ×3 |
| copper_ingot | metal | 169 | 0 | copper_ore ×3 |
| iron_ingot | metal | 124 | 0 | iron_ore ×3 |
| pure_gold | metal | 689 | 0 | gold_ore ×3 |
| steel_ingot | metal | 286 | 0 | iron_ingot ×2 |
| heavy_arms | weapon | 16500 | 0 | steel_ingot ×10, machine_parts ×5 |
| munitions | weapon | 416 | 0 | steel_ingot ×1, coal ×1 |
| small_arms | weapon | 1235 | 0 | steel_ingot ×2, fine_wood ×1, machine_parts ×1 |
| basic_clothing | essentials | 128 | 0 | fabric ×3 |
| basic_furniture | essentials | 300 | 0 | fine_wood ×3, machine_parts ×1 |
| simple_tools | essentials | 140 | 0 | iron_ingot ×1, fine_wood ×1 |
...
| fine_wood | component | 65 | 0 | wood_log ×2 |
| machine_parts | component | 286 | 0 | iron_ingot ×2 |
| precision_tools | component | 676 | 0 | steel_ingot ×1, machine_parts ×1 |
...
| car | vehicle | 6600 | 0 | engine ×1, steel_ingot ×2, electronic_components ×2, oil ×2, aluminum_ingot ×1 |
| tank | vehicle | 27500 | 0 | engine ×1, heavy_arms ×1, steel_ingot ×20, machine_parts ×5 |
...
|------|----------------|------------------|-------------|
| 초반 정착 | 0~5년 | 벌목/제재/경공업 운영 흑자 전환, `mining`~`components` 진입. **초반 노조 위기(50~100명)** 극복 | 적자 장기 고착이 아닌지, 연구 선택지가 1개로 고정되지 않는지 |
| 중반 확장 | 5~15년 | 금속-부품-엔진 라인 구축, `oil_extraction`~`engine_manufacturing` 완료. **중반 전쟁 위기(300~400명)** 돌파 | 연구-건설-인력 확장이 같은 속도로 따라가는지 |
| 중후반 전환 | 15~30년 | `heavy_industry`/`military_production` 선택적 진입, 고부가 상품 비중 증가 | 단일 품목 올인보다 포트폴리오 운영이 유리한지 |
| 장기 체제 | 30~50년+ | `vehicle_assembly` 포함한 고티어 체인 안정화, **후반 자동화 폭동 위기(1000명+)** 극복 | 시장 변동(뉴스/오더)에 대응 가능한 현금흐름 유지 여부 |
...
|----|------------|------------|----------------|
| worker | 10 | 100 | 0.5 |
| technician | 20 | 100 | 0.5 |
| researcher | 80 | 100 | 0.5 |
| manager | 120 | 100 | 0.5 |
...
- 플레이어 초기 자금(`InitialFinancesData.initialCredit`)은 `100000`.
...
- 현재 Company `baseWealth` 범위:
  - 최소 `136000`
  - 최대 `340000`
- 플레이어 초기 자금(`100000`) 대비 Company 하한이 충분히 높아 초반 경쟁 구도가 명확하며,
