...
public class DebugPopup : PopupBase
{
    ...
    private IEnumerator BuildDebugButtonsRoutine()
    {
        ...
        List<(string label, Action action)> buttonDefs = new List<(string label, Action action)>
        {
            ...
            ("Time +1 Day", () => DataManager.Instance.Time.AdvanceOneDay()),
            ("Time +1 Month", () => DataManager.Instance.Time.AdvanceOneMonth()),
            ("Time +1 Year", () => DataManager.Instance.Time.AdvanceOneYear()),
